...
export const useRoleManagement = (): UseRoleManagementReturn => {
    ...
    const fetchRoleDetails = useCallback(async (id: number) => {
        setFormLoading(true); // Reuse formLoading or add specific loading
        try {
            // We need to import getRoleByIdAPI
            // Assuming getRoleByIdAPI is available in ../services/roles
            // Use require/import dynamically if needed or just use what we have
            const { getRoleByIdAPI } = require('../services/roles');
